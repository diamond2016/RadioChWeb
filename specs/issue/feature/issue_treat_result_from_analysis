## issue feature: "treat result of analyzeStream (from stream_analysis_service) before persisting in db in table stream_analysis"

1. extract only the part of interest, "after Metadata";
2. apply security before saving data for sql injection (I think no necessary it is not entered by user)
3. In proposal_route /proposal (get) showing in a proper manner for styling in order to make text aligned and readable.

Example:
select * from stream_analysis;

1|http://78.129.202.200:8030|5|1|0||BOTH|HTTP/1.0 200 OK
...
Connection:close
icy-name:Radio Caroline
icy-genre:Classic Rock, Oldies
icy-br:96
icy-sr:22050
...
Input #0, aac, from 'http://78.129.202.200:8030':
  Metadata:
    icy-notice1     : <BR>This stream requires <a href="http://www.winamp.com">Winamp</a><BR>
    icy-notice2     : SHOUTcast DNAS/win64 v2.5.5.733<BR>
    icy-name        : Radio Caroline
    ...
    icy-url         : http://www.radiocaroline.co.uk
    icy-pub         : 1
    StreamTitle     : Bad Company - Silver, Blue & Gold
  Duration: N/A, bitrate: 95 kb/s
  Stream #0:0: Audio: aac (LC), 44100 Hz, stereo, fltp, 95 kb/s
Stream mapping:
...
[out#0/null @ 0x5617751cbb00] video:0KiB audio:172KiB subtitle:0KiB other streams:0KiB global headers:0KiB muxing overhead: unknown
size=N/A time=00:00:01.00 bitrate=N/A speed= 136x`   