...
Tests: Ran full suite — 20 failed, 38 passed. Main issues: request-context usage in services, missing RadioSourceService methods, DTO field mismatches, and some route/import gaps.
Branches/PR: feat/dto-migration pushed and PR opened (WIP) — https://github.com/diamond2016/RadioChWeb/pull/7. Stabilization & focused branches created (stabilize/dto-migration, feat/dto-migration-proposal-service, feat/dto-migration-radio-source-service).
What I changed: added DTO compatibility aliases, removed duplicate unit tests in unit, cleared caches, and ran tests.
When you’re ready tomorrow

I can resume and implement the prioritized fixes (start with RadioSourceService and request-context removal), run focused tests, then iterate until green.
Or I can prepare a compact patch plan you can apply.
...
## NOTES ON client_helpers
These are the two client helpers that make login to test user and test admin
in specs/conftest.py

```python
@pytest.fixture
def logged_in_client(test_app: app.Flask, test_user: User):
...
@pytest.fixture
def admin_logged_in_client(test_app: app.Flask, test_admin: User):
    """Create a test client logged in as an admin user."""
    with test_app.test_client() as client:
        with client.session_transaction() as sess:
...
This pytest fixture returns a Flask test client that is already logged in as the provided admin user.
It manipulates the test client's session directly so tests can exercise authenticated routes without going through the login form.

Step‑by‑step behavior

test_app.test_client() creates a Flask test client (context manager).
client.session_transaction() opens a temporary test request context and yields the session object used by that client.
sess['_user_id'] = str(test_admin.id) sets the user id key Flask-Login expects in the session.
sess['_fresh'] = True marks the session as a fresh login (so @fresh_login_required / some checks succeed).
yield client returns the client to the test. Because the yield happens inside the with blocks, the request/session contexts and client stay live for the duration of the test; after the test finishes the session is saved and the client/context is closed.

Gotchas and checks

This relies on the session keys Flask-Login uses by default ("_user_id" and "_fresh"). If you use a custom auth system or a different key, update accordingly.

session_transaction requires cookies enabled (test_client(use_cookies=True), default is True). If cookies are disabled you’ll get a TypeError.
app.session_interface.open_session must return a session; otherwise you'll get RuntimeError.
If you need exceptions to propagate to the test client, ensure app.testing = True on test_app.
If your auth uses additional session fields (roles, permissions, "remember" token), set those too.
Alternative (more explicit) fixture using flask_login.login_user

If your tests have access to the app context and flask_login, logging in via login_user inside a test request context can be clearer:

```python
# ...existing code...
def admin_logged_in_client(test_app: app.Flask, test_admin: User):
    """Create a test client logged in as an admin user using flask_login."""
    from flask import g
    from flask_login import login_user

    with test_app.test_client() as client:
        with test_app.test_request_context():
            # login_user sets up current_user and session appropriately
            login_user(test_admin)
            # Now persist the session into the test client cookies
            # by copying cookies from the context into the client via session_transaction.
            with client.session_transaction() as sess:
                # No-op: session already modified by login_user; this triggers save on exit
                pass
        yield client
# ...existing code...
```