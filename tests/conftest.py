...
def test_app():
    ...
    # Use an application context only for setup (create tables, initial data)
    # but do not keep an app context open across tests. Tests should open
    # their own `app.app_context()` (see `test_db` fixture) to avoid
    # app-context stack corruption during teardown.
    ...
    # Do not yield while an app context is active. Yield the app object
    # so other fixtures/tests can manage app contexts explicitly.
    yield app
...
@pytest.fixture
def test_user(test_db: scoped_session[Session]):
    """Create a persisted test user with role 'user'. Returns ORM User."""
    # Use unique email per fixture run to avoid UNIQUE constraint collisions
    unique_email = f"test_user_{uuid.uuid4().hex}@example.com"
    user = User(email=unique_email, hash_password='testhash', role='user')
    test_db.add(user)
    # Use flush instead of commit so the outer test transaction/savepoint
    # remains under test framework control. Committing inside the fixture
    # can leak state across tests and break isolation.
    test_db.flush()
    test_db.refresh(user)
    return user

@pytest.fixture
def test_admin(test_db: scoped_session[Session]):
    """Create a persisted test user with role 'admin'. Returns ORM User."""
    unique_email = f"test_admin_{uuid.uuid4().hex}@example.com"
    admin = User(email=unique_email, hash_password='adminhash', role='admin')
    test_db.add(admin)
    test_db.flush()
    test_db.refresh(admin)
    return admin

# I need the two client helper that make login to test user and test admin
@pytest.fixture
