...
from model.dto.stream_analysis import StreamAnalysisResult, ErrorCode, DetectionMethod
...
class TestStreamAnalysisService:
    ...
    @pytest.fixture
    def mock_stream_type_service(self):
    ...
    @pytest.fixture
    def analysis_service(self, mock_stream_type_service):
    ...
    def test_unsupported_protocol_rejection(self, analysis_service):
        """Test FR-004: RTMP/RTSP URLs are rejected."""
        rtmp_url = "rtmp://stream.example.com/live"

        result = analysis_service.analyze_stream(rtmp_url)

        assert not result.is_valid
        assert result.error_code == ErrorCode.UNSUPPORTED_PROTOCOL
        assert not result.is_secure

    def test_https_security_detection(self, analysis_service):
    ...
    def test_http_security_warning(self, analysis_service):
        ...
        with patch.object(analysis_service, '_analyze_with_curl') as mock_curl, \
            ...
            result = analysis_service.analyze_stream(http_url)

            assert not result.is_secure  # HTTP should be insecure
            assert result.is_valid       # But still valid

    @patch('subprocess.run')
    def test_ffmpeg_authoritative_over_curl(self, mock_run, analysis_service):
        ...
        result = analysis_service.analyze_stream("https://stream.example.com/test")
    ...
    def test_curl_header_extraction(self, analysis_service):
    ...
    def test_ffmpeg_output_parsing(self, analysis_service):
    ...
    def test_metadata_detection_icecast(self, analysis_service):
    ...
    def test_metadata_detection_shoutcast(self, analysis_service):
    ...
    @patch('subprocess.run')
    def test_timeout_handling(self, mock_run, analysis_service):
